...
RSpec.describe ResultVault, type: :model do
  ...
  describe 'methods' do
    begin
      [:success, :ok, :good, :pass, :passed, :succeeded].each do |name|
        context ":#{name}" do
    ...
    describe ":data=" do
      it "raises an exception when called" do
        expect{result.data = {test_1: 'tester'}}.to raise_error ArgumentError, /:data is a reserved keyword and may not be set directly./
      end
    end # :data=
